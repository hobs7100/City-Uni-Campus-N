...
**How to apply:** Whenever introducing a "list of instances derived from a schedule/grid" feature backed by a separate state table, audit that the state table's unique constraint and lookup/upsert queries use the full same key as the instance-dedup logic in the read API. For timetable attendance, require a concrete scheduled start/end slot on both reads and writes; never silently create an unslotted record for a date with no lecture. Combined allocations should union distinct eligible slots, then store each slot independently. Also check UI list `key={}` props use the full composite key, not just the coarser id — a coarser key causes duplicate React keys too.
