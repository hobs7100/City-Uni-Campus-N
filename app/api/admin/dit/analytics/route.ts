...
const params = z.object({ from_date: z.string().date().optional(), to_date: z.string().date().optional(),
  test_series_id: uuid.optional(), class_id: uuid.optional(), semester_id: uuid.optional(), course_id: uuid.optional() });
export async function GET(req: NextRequest) {
  ...
  const rows = await resultRows({ from:d.from, to:d.to, testSeriesId:parsed.data.test_series_id, classId:parsed.data.class_id, semesterId:parsed.data.semester_id, courseId:parsed.data.course_id });
  ...
  const filter_options = {
    course_id:[...new Map(rows.map(r=>[r.course_id,{id:r.course_id,title:r.course_title,code:r.course_code}])).values()],
    class_id:[...new Map(rows.map(r=>[r.class_id,{id:r.class_id,name:r.class_name,session:r.session}])).values()],
    semester_id:[...new Map(rows.map(r=>[r.semester_id,{id:r.semester_id,name:String(r.semester_number),term_type:r.term_type}])).values()],
    test_series_id:[...new Map(rows.map(r=>[r.test_series_id,{id:r.test_series_id,name:r.series_name}])).values()]
  };
  return NextResponse.json({ effective_filters:effective(raw,d.from,d.to), filter_options, subjects });
}