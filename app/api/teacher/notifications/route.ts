...
export async function GET(_request: NextRequest) {
  ...
  const notifications = await query(
    `select * from notifications where recipient_type = 'teacher' and recipient_id = $1 order by created_at desc limit 100`,
    [session!.userId]
  );
...
export async function PATCH(request: NextRequest) {
  ...
  if (parsed.data.mark_all) {
    await query(`update notifications set is_read = true where recipient_type = 'teacher' and recipient_id = $1`, [session!.userId]);
    return NextResponse.json({ success: true });
  }

  if (parsed.data.id) {
    const existing = await queryOne(
      `select id from notifications where id = $1 and recipient_type = 'teacher' and recipient_id = $2`,
