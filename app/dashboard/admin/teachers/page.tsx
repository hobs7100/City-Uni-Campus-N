...
type MainTab = "management" | "permanent" | "visiting";
...
export default function TeachersPage() {
  ...
  return (
    <div>
      ...
      {/* Main tab bar */}
      <div className="mb-6 flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        {mainTabs.map((t) => (
          <button
            key={t.id}
      ...
      {/* ── Permanent Tab ── */}
      {mainTab === "permanent" && (
        <TeacherWorkloadTabs apiEndpoint="/api/admin/teachers/workload" initialSubTab="permanent" />
      )}

      {/* ── Visiting Tab ── */}
      {mainTab === "visiting" && (
        <TeacherWorkloadTabs apiEndpoint="/api/admin/teachers/workload" initialSubTab="visiting" />
      )}
    </div>
  );
}
