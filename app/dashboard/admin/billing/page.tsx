...
export default function BillingPage() {
  ...
  const permTeacherOptions = useMemo(
    () => teachers.filter((t) => t.type === "permanent").map((t) => ({ value: t.id, label: `${t.name} (${departments.find((d) => d.value === t.department_id)?.label ?? ""})` })),
            ...
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {permLoading ? (
                  ...
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400">Select a teacher to preview billable lectures.</td></tr>
                ) : permItems.length === 0 ? (
