...
export default async function CoordinatorOverviewPage() {
    ...
    return (
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          You are not yet assigned as Coordinator for any department. Please contact the
          administrator.
        </p>
      </div>
    );
  ...
  const [students, classes] = await Promise.all([
    queryOne<{ count: string }>(
      `select count(*)::text as count from students where department_id = $1 and deleted_at is null`,
      [department.id],
    ),
        ...
        {cards.map((c) => (
          <div key={c.label} className="card-3d card-hover p-5">
            <div className="flex items-center gap-4">
              <div
                className={`flex h-11 w-11 items-center justify-center rounded-xl ${c.color} text-white`}
              >
                <c.icon size={20} />
              </div>
              <div>
