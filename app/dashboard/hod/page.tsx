...
export default async function HodOverviewPage() {
  ...
  const [students, teachers, classes] = await Promise.all([
    ...
    queryOne<{ count: string }>(`select count(*)::text as count from teachers where department_id = $1 and deleted_at is null`, [department.id]),
  ...
  const cards = [
    ...
    { label: "Teachers", value: Number(teachers?.count ?? 0), icon: UsersRound, color: "bg-rose-500" },
